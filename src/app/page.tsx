  ...
  return (
    <main className="min-h-screen bg-slate-50 p-4 md:p-12">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {produtos.map((item) => (
          <div key={item.id} className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-2xl transition-all group">
            <Link href={`/produto/${item.id}`} className="cursor-pointer block">
              ...
              <div className="p-6 text-center">
                <h3 className="text-lg font-bold text-slate-800 mb-2 group-hover:text-blue-600">
                  {item.nome}
                </h3>
                <p className="text-2xl font-black text-slate-900 mb-4">R$ {item.preco.toFixed(2)}</p>
                <button className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold uppercase text-xs hover:bg-blue-700 transition-all">
